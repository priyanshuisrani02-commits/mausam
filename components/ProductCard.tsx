...
export default function ProductCard({
  ...
  const needsSizeChoice = (availableSizes?.length ?? 0) > 1;
    ...
    <Link href={`/products/${id}`} className="block h-full">
      <article className="group h-full overflow-hidden rounded-[22px] border border-[#e4ddd2] bg-[#fffdf8] shadow-[0_7px_25px_rgba(70,61,45,0.055)] transition duration-500 hover:-translate-y-1 hover:shadow-[0_16px_34px_rgba(70,61,45,0.11)]">
        <div className="relative overflow-hidden bg-[#eee8dd]">
          <div className="relative aspect-[4/5] w-full md:aspect-auto md:h-[430px]">
            {image ? (
              <img
                src={image}
                alt={name}
                loading="lazy"
                decoding="async"
                className="h-full w-full object-cover transition duration-700 md:group-hover:scale-[1.045]"
              />
            ) : (
              <div className="flex h-full w-full items-center justify-center bg-[#eee8dd] px-6 text-center text-[10px] uppercase tracking-[2px] text-[#8a8378]">
                Image unavailable
              </div>
            )}
          </div>
          <div className="pointer-events-none absolute inset-0 bg-gradient-to-t from-[#302d27]/10 via-transparent to-transparent opacity-60" />
          ...
          >
            {needsSizeChoice ? "Choose options" : "Add to cart"}
          </button>
        </div>
