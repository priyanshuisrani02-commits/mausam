    ...
    <Link href={`/products/${id}`} className="block h-full">
      <article className="group h-full overflow-hidden rounded-[22px] border border-[#e4ddd2] bg-[#fffdf8] shadow-[0_7px_25px_rgba(70,61,45,0.055)] transition duration-500 hover:-translate-y-1 hover:shadow-[0_16px_34px_rgba(70,61,45,0.11)]">
        ...
        <div className="px-4 py-4 sm:px-5 sm:py-5">
          <h3 className="line-clamp-2 text-sm font-medium leading-5 text-[#3c3932] sm:text-base">
            {name}
          </h3>
          <div className="mt-2 flex items-center gap-2">
            <span
              className={
                salePrice != null
                  ? "font-medium text-[#a45b3f]"
                  : "font-medium text-[#4d493f]"
              }
            >
              ₹{displayPrice.toLocaleString("en-IN")}
            </span>
            {salePrice != null && (
              <span className="text-xs text-[#9a9287] line-through">
